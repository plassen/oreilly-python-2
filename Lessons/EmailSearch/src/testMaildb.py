...
class testRealEmail_traffic(unittest.TestCase):
    def setUp(self):
        ...
        for f in files:
            ...
            idee = self.msgids[msg['message-id']] = maildb.store(msg)
            self.message_ids[idee] = msg['message-id']
            date = msg['date']
    ...
    def test_ids(self):
        ...
        """
        for idee in self.message_ids.keys():
            pk, msg = maildb.msg_by_message_id(self.message_ids[idee])
            self.assertEqual(msg['message-id'], self.message_ids[idee])
