...
email maessages using a MySQL relational database.
"""
...
def msg_by_id(idee):
    ...
    """
    curs.execute("SELECT msgID, msgText FROM message WHERE msgID=%s", (idee,))
    result = curs.fetchone()
    if not result:
        raise KeyError("Id {0} not found in store".format(idee))
    idee, text = result
...
def msgs(mindate=None, maxdate=None, namesearch=None, addsearch=None):
    ...
    addsearch is given, the resilt set is restricted to messages with email
    ...
    if namesearch:
        ...
        data.append("%s" + namesearch.strip().lower() + "%s")
    sql = "SELECT msgid, msgText FROM message"
    if conds:
        sql += " WHERE " + " AND ".join(conds)
    curs.execute(sql, tuple(data))
    result = []
    for idee, text in curs.fetchall():
        result.append((idee, message_from_string(text)))
    return result